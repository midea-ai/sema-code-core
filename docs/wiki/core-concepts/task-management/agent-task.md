...
| 模式 | 触发 | TaskManager 调用 | 是否占用当前会话主对话 |
|------|------|------------------|--------------|
| 前台 SubAgent | `sub_agent` 工具 `background: false`（默认）| `registerForegroundAgent` | 当前会话主对话等待结果 |
...
sub_agent 工具
   ...
   │
   ▼
TaskManager.spawnAgentTask(taskId, sessionId, title, toolUseId, executeFn, agentConfig.name)
   │
   ├─ 检查 MAX_RUNNING_TASKS（按会话独立计算）
   ├─ 创建独立 AbortController（与主代理 AC 完全无联动）
   ├─ ensureTaskDir() + 创建空输出文件
   ├─ 创建 TaskRecord（type: 'SubAgent', sessionId, _abortController, agentType）
   ├─ emit task:start { taskId, command: description, filepath: '', status, type: 'SubAgent', agentType }（按 sessionId 路由）
   │
   ...
   │     └─ 任意分支 finally：runtime.forAgent(taskId).clearAllState()
   │
   └─ Promise.then / catch：
...
当前会话主代理可继续处理其它工作。后台 SubAgent 完成时通过 `_notify` 注入任务归属会话。
...
```javascript
const sharedAbortController = runtime.currentAbortController  // 当前会话主代理 AC
...
taskManager.registerForegroundAgent(
  taskId, sessionId, title, toolUseId, subAbortController, unlinkAbort, agentConfig.name,
)
runtime.addForegroundAgent(taskId)
```

前台 SubAgent 在 `tasks` Map 中创建一条 `foreground: true` 的记录，**不出现在 `getTaskList()` 中**（避免 UI 误以为有"游离"任务），但会占用该会话一个 `MAX_RUNNING_TASKS` 名额。
...
| 结果 | 处理 |
|------|------|
| `completed` | `taskManager.finalizeTask(taskId, 0, result, usage)`，工具 yield 完整结果给当前会话主对话 |
...
`session.transferAgentToBackground(taskId)` → `TaskManager.transferToBackground(taskId)`：
...
`session.transferAllForegroundAgents()` 会按当前会话过滤 `foreground && running` 的 record，并依次调用 `transferToBackground`。
...
finalizeTask(taskId, exitCode, output?, usage?) {
  ...
  this._finishTask(record, exitCode)            // → emit task:end + 通知（仅非前台），结束后清空 output 释放内存
}
```

> `runtime.removeForegroundAgent(taskId)` 在 `executionPromise` 的 `finally` 块中调用，确保无论前台完成还是转后台结束都会清理状态。

`_finishTask` 内部判断：**前台任务不发通知**（结果由 `SubAgent.ts` 直接 yield 回当前会话主对话）；只有"转后台后完成"或"直接 spawnAgentTask"的任务才会触发 `_notify`。

## 中断行为

| 入口 | 实际效果 |
|------|---------|
| `session.interrupt()` | 当前会话主 AC abort → 联动到前台子 AC → 子代理在最近的检查点中止；后台 SubAgent **不受影响** |
| `stopTask(taskId)` | 直接 `record._abortController.abort()`，无论前/后台都会中止；同时 emit `task:end` 与（后台任务的）通知 |
| `session.dispose()` / `core.closeSession(sessionId)` | 调用 `disposeSession(sessionId)`，停止并清理该会话的 running 任务与 watcher |
...
> 与 RunShell 任务不同，SubAgent 通知直接携带完整 `<result>`，目标会话的主代理无需再读输出文件。
...
```javascript
session.getTaskList()                       // 当前会话后台任务，不含前台 SubAgent
session.watchTask(taskId, onDelta)          // SubAgent 任务的 output 一般在结束时一次性写入
session.stopTask(taskId)                    // 中止指定运行中的 SubAgent
session.transferAgentToBackground(taskId)   // 把单个前台 SubAgent 转后台
session.transferAllForegroundAgents()       // 把当前会话所有前台 SubAgent 批量转后台
```

典型场景：用户在 UI 上看到当前会话主代理调起了一个耗时较长的子代理，决定让它"在后台跑"，当前会话主对话立刻回到 idle —— 这就是 `transferAgentToBackground` 的目标用法。
