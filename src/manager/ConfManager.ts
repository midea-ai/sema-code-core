...
import { readInitialCwd, updateInitialCwd } from '../util/cwd';
...
export class ConfigManager {
  ...
  /**
   * 更新禁用的工具列表（黑名单），内部转换为白名单存入 useTools
   */
  updateDisabledTools(toolNames: string[] | null): void {
    ...
    this.coreConfig = this.applyDisabledTools({ ...this.coreConfig, disabledTools: toolNames });
    logInfo(`禁用工具列表已更新: ${toolNames && toolNames.length > 0 ? toolNames.join(', ') : 'null (不禁用)'}`);
  }

  /**
   * 将 disabledTools（黑名单）转换为 useTools（白名单）；同时传入时 disabledTools 优先
   */
  private applyDisabledTools(config: SemaCoreConfig): SemaCoreConfig {
    const { disabledTools, ...rest } = config;
    if (disabledTools === undefined) return config;
    // 懒加载避免与 tools.ts 形成循环依赖
    const { getAllBuiltinToolNames } = require('../tools/base/tools') as typeof import('../tools/base/tools')
    const useTools = disabledTools && disabledTools.length > 0
      ? getAllBuiltinToolNames().filter(name => !disabledTools.includes(name))
      : null;
    return { ...rest, useTools };
  }
