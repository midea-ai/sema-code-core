 ...
 * 实现优先级：内置 < 插件 < 用户级 < 项目级 (后加载的覆盖先加载的)
 */
...
class AgentsManager {
  ...
  constructor() {
    ...
    // 后台静默加载 agents 信息
    this.loadingPromise = this.loadAndCacheAgents()
      .catch(err => {
  ...
  private async parseAgentFile(filePath: string): Promise<AgentConfig | null> {
    try {
      ...
      // 把外部代码助手的工具名规范化为内置工具名（仅 搜索、文件读写和终端 等少量映射）
      if (Array.isArray(tools)) {
        const aliasHits: string[] = []
        tools = tools.map(t => {
          if (isToolAlias(t)) {
            const canonical = normalizeToolName(t)
            aliasHits.push(`${t} -> ${canonical}`)
            return canonical
          }
          return t
        })
        if (aliasHits.length > 0) {
          logDebug(`Agent [${name}] 工具名映射: ${aliasHits.join(', ')}`)
        }
      }

      // model 只取字符串类型，非字符串忽略，回退默认值 'quick'
  ...
  async getAgentsInfo(concise?: boolean, refresh?: boolean): Promise<AgentConfig[]> {
    ...
    if (refresh) {
      infos = await this.loadAndCacheAgents()
    } else if (this.agentInfoCache) {
      infos = this.agentInfoCache
  ...
  async addAgentConf(agentConf: AgentConfig): Promise<AgentConfig[]> {
    ...
    return this.loadAndCacheAgents()
  }
