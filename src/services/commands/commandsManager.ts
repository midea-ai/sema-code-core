...
class CommandsManager {
  ...
  constructor() {
    ...
    // 后台静默加载 commands 信息
    this.loadingPromise = this.loadAndCacheCommands()
      .catch(err => {
  ...
  private async loadCommandsFromDir(dirPath: string, scope: 'user' | 'project'): Promise<void> {
    try {
      ...
      if (loadedCount > 0) {
        logDebug(`加载 ${scope} 级 Commands: ${loadedCount} 个`)
      }
    } catch (error) {
      logError(`加载 ${scope} 级 Commands 失败 [${dirPath}]: ${error}`)
    }
  }
  ...
  private async scanMdFiles(dirPath: string): Promise<string[]> {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        results.push(await this.scanMdFiles(fullPath))
  ...
  async getCommandsInfo(concise?: boolean, refresh?: boolean): Promise<CommandConfig[]> {
    ...
    if (refresh) {
      infos = await this.loadAndCacheCommands()
    } else if (this.commandInfoCache) {
      infos = this.commandInfoCache
  ...
  async addCommandConf(commandConf: CommandConfig): Promise<CommandConfig[]> {
    ...
    return this.loadAndCacheCommands()
  }
