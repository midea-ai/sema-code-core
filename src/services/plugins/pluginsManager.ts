...
class PluginsManager {
  ...
  private async readMarketplaceJson(installLocation: string): Promise<MarketplaceJson | null> {
    try {
      const results = await searchWithRipgrep(
        ['--files', '--glob', 'marketplace.json', '--max-depth', '3', '--hidden'],
        installLocation
      )
      if (results.length === 0) {
        logWarn(`marketplace.json 不存在: ${installLocation}`)
        return null
      }
      const content = await fsPromises.readFile(results[0], 'utf8')
   ...
   */
  private async loadAllEnabledPlugins(): Promise<Record<PluginScopeKind, Record<string, boolean>>> {
  ...
  private async gitCloneMarketplace(repo: string, targetDir: string): Promise<void> {
    const { code, stderr } = await execFileSafely(
  ...
  private async getGitCommitSha(repoDir: string): Promise<string> {
    const { code, stdout } = await execFileSafely(
  ...
  /**
   * 读取插件目录下的 plugin.json（通过 ripgrep 搜索，不依赖固定目录名）
   */
  private async readPluginJson(pluginSourcePath: string): Promise<PluginJson | null> {
    try {
      const results = await searchWithRipgrep(
        ['--files', '--glob', 'plugin.json', '--max-depth', '3', '--hidden'],
        pluginSourcePath
      )
   ...
   * 2. 插件目录下 plugin.json 的 version 字段
   * 3. 默认 '1.0.0'
   */
  ...
  private async buildMarketplaceResult(
    ...
    enabledPluginsMap: Record<PluginScopeKind, Record<string, boolean>>
  ...
  async refreshMarketplacePluginsInfo(): Promise<MarketplacePluginsInfo> {
    ...
    const result = await this.buildMarketplaceResult(known, installed, enabledPluginsMap)
    ...
    setImmediate(() => {
      ...
      import('../skills/skillsManager').then(({ getSkillsManager }) => {
        getSkillsManager().getSkillsInfo(undefined, true).catch((err: unknown) => logError(`插件变更后刷新 Skills 失败: ${err}`))
      }).catch(() => {})
      import('../commands/commandsManager').then(({ getCommandsManager }) => {
        getCommandsManager().getCommandsInfo(undefined, true).catch((err: unknown) => logError(`插件变更后刷新 Commands 失败: ${err}`))
      }).catch(() => {})
