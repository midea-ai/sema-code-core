 ...
 */
export async function handlequickchat(question: string, sessionId: string): Promise<void> {
  ...
  const mainAbort = runtime.currentAbortController
  ...
  try {
    ...
    getEventBus().emit('quickchat:response', { question, content }, sessionId)
  } catch (error) {
